...
use crate::mit_protocol::{self, MitFeedback, MotorLimits};
...
/// Runs on a background thread: owns a SocketCAN socket, forwards outgoing frames,
/// and parses incoming frames according to `protocol` into `FromCanWorker` events.
pub fn run(
    iface: String,
            ...
            Ok(CanFrame::Data(frame)) => {
                ...
                match &protocol {
                    CanProtocol::Mit(limits) => {
                        // MIT feedback uses a standard (11-bit) id; servo-mode telemetry uses an
                        // extended (29-bit) id. Without this check, servo-mode frames (which are
                        // also 8 bytes) get misread as MIT feedback and produce bogus numbers.
